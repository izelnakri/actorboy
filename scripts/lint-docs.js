...
//   (WebSocketServer from ws, in lib/node/hub.ts). These can't be fixed without re-exporting
//   third-party types, which would bloat the public API.
...
const [gateAlive, docLint, ignoreFences, exampleGaps] = await Promise.all([
  doctestGateIsAlive(),
  run('deno', ['doc', '--lint', '--quiet', 'lib/', 'mod.ts']),
  ...
  run('grep', ['-rnE', '```(ts|js)[^`]*\\bignore\\b', '--include=*.ts', 'lib/', 'mod.ts']),
  missingExamples(),
]);
