...
/**
 * Benchmark regression checker for actorboy.
 *
 * Modes
 * -----
 * Default (stdin)
 ...
 *   one group (e.g. Stream's per-element allocation churn) cannot inflate
 *   latencies in another (Result's zero-allocation happy path, where the whole
 *   claim is measured in nanoseconds). Combines the per-file JSON before
 *   checking. Tighter multipliers are safe because cross-file GC interference
 *   is eliminated.
 *
 * --save
 ...
 *                         the whole gate.  Useful when laptop load makes the
 *                         cluster benches falsely regress; CI keeps the strict gate.
 *                         Examples:
 *                           SKIP_BENCHMARK=true         # skip everything
 *                           SKIP_BENCHMARK=node         # skip benches/node.bench.ts
 *                           SKIP_BENCHMARK=node,stream  # skip multiple files
 *
 * Usage
 * -----
 *   # isolated (preferred):
 *   deno run --allow-all scripts/check-benchmarks.ts \
 *     --files benches/result.bench.ts benches/task.bench.ts ...
 *
 *   # pipe-based (legacy):
...
// Colours are inlined rather than imported from @std/fmt: this package ships with zero
// runtime dependencies and the same discipline is cheap to keep in its tooling. NO_COLOR /
// FORCE_COLOR=0 are honoured because CI sets the latter and a table full of escape codes in
// a log is worse than no colour at all.
const COLOR =
  !Deno.env.get('NO_COLOR') && Deno.env.get('FORCE_COLOR') !== '0' && Deno.stdout.isTerminal();
...
function paint(open: number, close: number, text: string): string {
  return COLOR ? `\x1b[${open}m${text}\x1b[${close}m` : text;
}

function bold(text: string): string {
  return paint(1, 22, text);
}

function dim(text: string): string {
  return paint(2, 22, text);
}

function red(text: string): string {
  return paint(31, 39, text);
}

function green(text: string): string {
  return paint(32, 39, text);
}

function yellow(text: string): string {
  return paint(33, 39, text);
}

function gray(text: string): string {
  return paint(90, 39, text);
}
 ...
 *   1ms–100ms  1×    pure CPU work (a large Stream pipeline, a Raft log replay) — stable,
 *                    GC-bound but contained.
 *   ≥ 100 ms   2.5×  spawn-or-larger: worker-thread startup, socket handshakes, a cluster
 *                    converging. OS-bound and load-sensitive; flat 1× had no headroom over
