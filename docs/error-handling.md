...
Tests: [`test/result/`](../test/result/), [`test/task/`](../test/task/).
...
check while being the same error in every sense that matters. That is not exotic here: a node
ships declared failures to its peers over a WebSocket and to a `workerPool` over
`worker_threads`, so every failure crossing those links is cross-realm by construction _and_
serialized, which destroys the prototype anyway.
...
typed envelope. `lib/node`'s wire protocol does the latter, carrying a declared failure in an
explicit `$failure` slot on the frame rather than asking the payload what it is.
...
```ts
// a legitimate producer whose VALUE is a fetched failure report — a supervisor collecting
// why its children died, revived from the wire with Failure.fromJSON
...
Reach for it when the failure is part of the contract: a `call` whose caller must tell a timeout
from a refusal, a supervisor deciding whether to restart, a library whose users need to branch on
why. Everywhere else, let bugs throw.
