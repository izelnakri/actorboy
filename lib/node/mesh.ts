...
export function webSocketMeshLink(url: string, codec: Codec = binaryCodec): MeshLink {
  ...
  // `<ArrayBuffer>`, not a bare `Uint8Array`: `WebSocket.send` refuses a SharedArrayBuffer-backed
  // view, which is exactly what the wide type admits. `Codec.encode` already narrows this way.
  const backlog: (string | Uint8Array<ArrayBuffer>)[] = [];
