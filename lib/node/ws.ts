...
export interface Codec {
  // The `<ArrayBuffer>` on the way out is not pedantry: `WebSocket.send` refuses a
  // SharedArrayBuffer-backed view, so a codec that produced one could not be sent at all.
  // Inbound stays wide — whatever the socket hands us is data we must read.
  /** Frame → wire bytes (binary codecs) or text (JSON codecs). */
  encode(frame: Frame): Uint8Array<ArrayBuffer> | string;
  /** Wire data → frame. */
