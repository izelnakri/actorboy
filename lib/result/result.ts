 ...
 * const parse = (argv: string[]): Result.Result<string[], Failure.Of<typeof ParseFailure>> =>
 *   argv.includes('--nope') ? ParseFailure({ flag: '--nope' }) : argv;
 *
 * Result.unwrap(parse(['--watch'])); // ['--watch'], or the ParseFailure throws
 * ```
 */
