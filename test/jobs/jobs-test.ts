...
module('Jobs | Oban-shaped durable queue', () => {
  ...
  test('errors capture the stack; a thrown Failure adds its code + safe data, a bad payload is dropped', async (assert) => {
    ...
    assert.equal(
      ...
      'RateLimited',
      'a declared throw keeps its code — for dead-letter routing',
    );
    assert.deepEqual(
      typedError.error.data,
      { retryAfter: 30 },
      'and its data — the full Failure API, not a flattened copy',
    );
    ...
    assert.equal(badError.error.code, 'RateLimited', 'still captures the code');
    jobs.stop();
  });
  ...
  test('a failed attempt with tries left parks in `retryable`, its next run scheduled by backoff', async (assert) => {
    ...
    assert.equal(
      retryable.errors[0].error.message,
      'boom',
      'the failure is recorded on the job, as a live Failure',
    );
    jobs.stop();
  });
