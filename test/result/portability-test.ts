...
// Result, Failure, Task and Stream are the universal abstractions of this codebase: they must load
...
module('Result | portability', { concurrency: true }, () => {
  test('the Result/Failure/Task graph loads and works without a process global', async (assert) => {
    ...
    const script =
      ...
      `  streamed.values.length, streamed.errors.length, total);`;
    ...
    assert.true(stdout.includes('OK 1 42 true FileMissing 2 1 5'), 'the full surface worked');
    assert.true(
