...
function route(req: IncomingMessage): Promise<Reply> {
  ...
  // The two `async` recipes below have nothing to await: this toy Task takes `() => Promise<T>`
  // (the production one in lib/task/ accepts either), so `async` IS the way to hand it a plain
  // value here. require-await is reading the keyword, not the contract.
